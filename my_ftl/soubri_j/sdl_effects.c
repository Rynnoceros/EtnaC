...
void load_sound_lib(play_screen_t *the_game)
{
    if (the_game != NULL) {
        ...
        the_game->sound_lib[2] = load_sound("control", "sounds/control.wav");
        the_game->sound_lib[3] = load_sound("getbonus", "sounds/getbonus.wav");;
        the_game->sound_lib[4] = load_sound("jump", "sounds/jump.wav");;
        the_game->sound_lib[5] = load_sound("repair", "sounds/repair.wav");
        the_game->sound_lib[6] = load_sound("attack", "sounds/attack.wav");
        ...
        for (int i = 0; i < 10; ++i) {
            if (the_game->sound_lib[i] != NULL) {
                if (my_strcmp(the_game->sound_lib[i]->name, sound_name) == 0) {
                    SDL_QueueAudio(the_game->sound_lib[i]->audio_id, 
                        the_game->sound_lib[i]->wav_buffer, 
                        the_game->sound_lib[i]->wav_length);
